-- Fully consolidated migrations for resume and social functionality

-- 1. Create resumes table with all fields including social links
...
-- 5. Create function to match resume content using embeddings
...
-- 6. Create function to match resumes based on full text
CREATE OR REPLACE FUNCTION match_resume_text(
  query_text TEXT,
  user_id UUID
)
RETURNS TABLE (
...
BEGIN
  ...
  SELECT
    id as resume_id,
    similarity(full_text, query_text) as similarity,
    substring(full_text, 1, 200) as content
  FROM
    public.resumes
  WHERE
    user_id = match_resume_text.user_id
  AND
...
-- 7. Add useful index for user_id to optimize queries
