...
class AuthAdapter:
    ...
    async def verify_token(self, token: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                is_test_token = (
                    user_id.startswith("test-") or 
                    "test@" in payload.get("email", "") or
                    settings.DEV_MODE
                )
                ...
                if not user_profile:
                    if is_test_token or settings.is_development:
                        logger.warning(f"🔐 User profile not found for ID: {user_id}, creating test profile")
                        
                        # Check if token has user_metadata with user_type
                        user_type = "job_seeker"
                        if payload.get("user_metadata") and isinstance(payload["user_metadata"], dict):
                        ...
                        logger.warning(f"🔐 User profile not found for ID: {user_id} and not in development mode")
                        return None
                ...
                return {
                    ...
                    "is_test_user": user_profile.get("is_test_profile", False)  # Flag for test users
                }
    ...
    async def _get_user_with_role(self, user_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                admin_result = client.table("admin_profiles").select("*").eq("user_id", user_id).execute()
            ...
            try:
                partner_result = client.table("partner_profiles").select("*").eq("id", user_id).execute()
            ...
            try:
                job_seeker_result = client.table("job_seeker_profiles").select("*").eq("id", user_id).execute()
    ...
    async def create_session(self, user_id: str, user_data: Dict[str, Any] = None) -> Optional[str]:
        ...
        try:
            ...
            session_data = {
                ...
                "status": "active",
                "data": {
                    "user_type": user_data.get("user_type", "job_seeker") if user_data else "job_seeker",
            ...
            result = await self.supabase_adapter.store_database_record("workflow_sessions", session_data)
            if result["success"]:
