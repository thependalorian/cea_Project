...
async def get_user_profile(
    ...
    try:
        ...
        if profile_type:
            ...
            result = await supabase.table(table_name).select("*").eq("user_id", user_id).limit(1).execute()
        ...
        for i, table in enumerate(tables):
            result = await supabase.table(table).select("*").eq("user_id", user_id).limit(1).execute()
