...
from ..models.user_model import UserProfile, JobSeekerProfile, PartnerProfile, AdminProfile
...
# Initialize production auth adapter
auth_adapter = AuthAdapter()

# Import Pendo workflow for session tracking
try:
    from ..workflows.pendo_supervisor import create_pendo_supervisor_workflow
...
async def verify_token(credentials: HTTPAuthorizationCredentials = Depends(security)) -> TokenData:
    ...
    try:
        ...
        return TokenData(
            ...
            user_type=token_data.get("user_type", "job_seeker"),
...
async def get_current_user(token_data: TokenData = Depends(verify_token)) -> UserProfile:
    ...
    try:
        ...
        if not user_profile_data and settings.is_development:
            logger.info(f"🧪 Creating test profile for user {token_data.user_id} in development mode")
            user_profile_data = {
        ...
        if not is_test_profile and PENDO_AVAILABLE and pendo_workflow:
            try:
                ...
                session_conversation_id = f"session_{token_data.user_id}_{datetime.utcnow().strftime('%Y%m%d')}"
...
def require_user_type(allowed_types: List[UserType]):
    ...
    def dependency(current_user: UserProfile = Depends(get_current_user)):
        if current_user.user_type not in allowed_types:
            logger.warning(f"Access denied for user {current_user.id} with type {current_user.user_type}")
            raise HTTPException(
...
async def get_auth_status(current_user: UserProfile = Depends(get_current_user)) -> AuthStatus:
    """
    Get current authentication status with Pendo integration
    ...
    try:
        ...
        # Get Pendo session info if available
        pendo_session_id = None
        if PENDO_AVAILABLE and pendo_workflow:
            try:
                session_stats = pendo_workflow.get_session_stats(current_user.id)
                pendo_session_id = f"pendo_{current_user.id}_{datetime.utcnow().strftime('%Y%m%d')}"
            except Exception as e:
                logger.warning(f"Failed to get Pendo session info: {e}")
        ...
        return AuthStatus(
            ...
            session_expires_at=None,  # TODO: Extract from JWT
...
async def get_user_permissions(current_user: UserProfile = Depends(get_current_user)) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if PENDO_AVAILABLE and pendo_workflow:
            try:
                ...
                user_sessions = {k: v for k, v in pendo_workflow.session_tracking.items() 
                ...
                logger.info(f"🧠 Cleared {len(user_sessions)} Pendo sessions for user {current_user.id}")
...
async def get_job_seeker_profile(current_user: UserProfile = Depends(get_job_seeker)) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        return {
            "profile": profile,
            "preferences": preferences_result["data"][0] if preferences_result["success"] and preferences_result["data"] else None,
            "recent_conversations": conversations_result["data"] if conversations_result["success"] else [],
            "user_type": "job_seeker",
            "dashboard_permissions": await auth_adapter.get_user_permissions(current_user.id, "job_seeker")
        }
...
async def get_partner_profile(current_user: UserProfile = Depends(get_partner)) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "user_type": "partner",
            "dashboard_permissions": await auth_adapter.get_user_permissions(current_user.id, "partner")
        }
...
async def get_admin_profile(current_user: UserProfile = Depends(get_admin)) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            # Count job seekers
            js_result = await supabase_adapter.query_database("job_seeker_profiles", select="count(*)")
            if js_result["success"]:
                users_stats["job_seekers"] = len(js_result["data"])
            
            # Count partners  
            partner_result = await supabase_adapter.query_database("partner_profiles", select="count(*)")
            if partner_result["success"]:
                users_stats["partners"] = len(partner_result["data"])
            
            # Count admins
            admin_result = await supabase_adapter.query_database("admin_profiles", select="count(*)")
        ...
        return {
            ...
            "user_type": "admin",
            "dashboard_permissions": await auth_adapter.get_user_permissions(current_user.id, "admin")
        }
...
async def get_dashboard_data(current_user: UserProfile = Depends(get_current_user)) -> Dict[str, Any]:
    """
...
async def debug_token(credentials: HTTPAuthorizationCredentials = Depends(security)) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if len(parts) == 3:
            ...
            if "sub" in payload:
                safe_payload["sub"] = f"{payload['sub'][:5]}...{payload['sub'][-5:]}" if len(payload["sub"]) > 10 else payload["sub"]
    ...
    debug_info["jwt_secret_preview"] = f"{secret[:5]}...{secret[-5:]}" if len(secret) > 10 else secret
