        ...
        if response.data:
            ...
            for job in response.data:
                formatted_job = {
                    ...
                    "description": job.get(
                        "description", "Climate economy opportunity"
                    ),
...
async def analyze_skills_gap(
    user_id: str, target_job_title: str = "climate economy role"
) -> str:
    """
    ...
    try:
        ...
        if response.data and len(response.data) > 0:
            ...
            gap_score = (
                matched_skills / total_climate_skills
                if total_climate_skills > 0
                else 0.0
            )
    ...
    try:
        ...
        if not user_message:
            user_message = state.get(
                "query", "I need help with climate career opportunities"
            )
        ...
        assistant_message = AIMessage(
            ...
            additional_kwargs={
                ...
                "confidence": jasmine_response.get("metadata", {}).get(
                    "confidence", 0.95
                ),
                "tools_used": jasmine_response.get("metadata", {}).get(
                    "tools_used", []
                ),
                "act_partners_referenced": jasmine_response.get("metadata", {}).get(
                    "act_partners_referenced", 0
                ),
                "has_resume": jasmine_response.get("metadata", {}).get(
                    "has_resume", False
                ),
            },
        )
...
async def process_user_resume(state: AgentState) -> AgentState:
    ...
    try:
        ...
        try:
            ...
            print(
                f"   ✅ Resume found: {resume.get('file_name', 'Unknown')} (ID: {resume_id})"
            )
        ...
        if (
            ...
            if resume.get("skills_extracted"):
                state["resume_analysis"] = {
                    ...
                    "experience_years": resume.get("experience_years", 0),
                    "education_level": resume.get("education_level", "Unknown"),
                    "climate_relevance_score": resume.get(
        ...
        if resume.get("content") and len(resume.get("content", "").strip()) > 100:
            ...
            print(
                f"   ✅ Using existing content from database: {len(content)} characters"
            )
        ...
        # If no existing content, try to download from storage
            ...
            try:
                ...
                file_response = supabase.storage.from_("resumes").download(
                    resume["file_path"]
                )
                ...
                if file_response:
                    ...
                    if "pdf" in content_type.lower():
                        ...
                        content = f"[Word Document Content from {resume.get('file_name', 'resume.docx')}]"
                        extraction_method = "storage_word"
                    else:
                        content = (
                            f"[File Content from {resume.get('file_name', 'resume')}]"
                        )
        ...
        if not content or len(content.strip()) < 50:
            ...
            content = (
                f"Resume for user {user_id} - {resume.get('file_name', 'Unknown file')}"
            )
...
"""
        ...
        print(
            f"   - Climate Relevance: {analysis_result.get('climate_relevance_score', 0.0):.2f}"
        )
        ...
        update_data = {
            ...
            "climate_relevance_score": analysis_result.get(
                "climate_relevance_score", 0.0
            ),
        ...
        try:
            ...
            try:
                supabase.table("resumes").update(minimal_update).eq(
                    "id", resume_id
                ).execute()
...
async def route_request(state: AgentState) -> str:
    ...
    if state.get("resume_data") and state.get("resume_data", {}).get(
        "has_resume", False
    ):
...
async def create_agent_graph():
    ...
    workflow.add_node(
        "environmental_justice_specialist", environmental_justice_specialist_handler
    )
