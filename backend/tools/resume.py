...
async def process_resume(
    ...
    try:
            ...
            try:
                ...
                if file_content:
                    ...
                    if "pdf" in content_type.lower():
                        ...
                        "word" in content_type.lower() or "docx" in content_type.lower()
                    ):
        ...
        analysis_result = await analyze_resume_with_llm(
            content, resume.get("file_name", "")
        )
        ...
        if embeddings and content:
            try:
                content_embedding = embeddings.embed_query(
                    content[:8000]
                )  # Limit content size
...
async def extract_skills_from_resume(content: str) -> List[str]:
    ...
    try:
        # Initialize the LLM
        llm = ChatOpenAI(
            model="gpt-3.5-turbo", temperature=0.2, api_key=settings.OPENAI_API_KEY
        )
        ...
        if "[" in skills_text and "]" in skills_text:
            ...
            skills_json = skills_text[
                skills_text.find("[") : skills_text.rfind("]") + 1
            ]
            ...
            try:
                ...
                if linkedin_url or github_url or personal_website:
                    social_result = await web_search_for_social_profiles(
                        ...
                        links=[
                            url
                            for url in [linkedin_url, github_url, personal_website]
                            if url
                        ],
                    )
                    social_data = (
    ...
    try:
        ...
        if "{" in analysis_text and "}" in analysis_text:
            json_str = analysis_text[
                analysis_text.find("{") : analysis_text.rfind("}") + 1
            ]
